# STEERING.md — Quaero (project-specific guidance)

> **Role of this file.** This is the **Quaero-specific** steering doc
> (product context: locked stack, conventions, roadmap, out-of-scope).
> It was **relocated here from `AGENTS.md`** because `AGENTS.md` / `CLAUDE.md`
> at the repo root now hold the **AI-DLC (AWS) workflow** — pure — which
> drives the development life cycle (Inception → Construction → Operations).
>
> **Division of authority:**
> - **AI-DLC** (`AGENTS.md` / `CLAUDE.md` + `.aidlc-rule-details/`) dictates the
>   *process*. On process conflicts, AI-DLC wins.
> - **This STEERING.md** dictates the *product/engineering context*. On stack &
>   product decisions, this doc wins.
>
> **Known correction.** The text below originally referenced `specs/PRD.md`.
> The PRD actually lives at the **repo root** (`/PRD.md`). The `specs/` folder
> is the intended home for SDD specs (one per feature) but does not exist yet.
>
> Steering doc for any AI (Claude Code, opencode, etc.) working in this repo.
> `CLAUDE.md` is a symlink to `AGENTS.md` (which now contains the AI-DLC
> workflow). For Quaero product context, read **this** file before touching code.
...
100% locally, ~zero cost, no deploy. Details in [`/PRD.md`](../PRD.md).

## Methodology: AI-DLC + Spec Driven Development

> **Update:** the formal **AWS AI-DLC workflow** is now installed at the repo
> root (`AGENTS.md`/`CLAUDE.md`). The Quaero-specific layer below (Bolts + SDD)
> coexists with it: AI-DLC is the outer loop, Bolts are the Quaero delivery
> cadence inside Construction. Invoke AI-DLC by starting a request with
> **"Using AI-DLC, ..."**.
